...
import { Property, Location, TxData } from './app.models';
...
export class AppService {
  ...
  public getRelatedProperties(): Observable<TxData>{
    // return this.http.get<TxData>(this.url + 'related-properties.json');
    //changed 
    return this.http.get<TxData>(this.url + 'properties.json');
  }

  public getPropertiesByAgentId(agentId): Observable<TxData>{
    // return this.http.get<TxData>(this.url + 'properties-agentid-' + agentId + '.json');
    //changed
    return this.http.get<TxData>(this.url + 'properties.json');
  }
  ...
  public addToCompare(property:Property, component, direction){ 
    if(!this.Data.compareList.filter(item=>item.ListingId == property.ListingId)[0]){
  ...
  public addToFavorites(property:Property, direction){
    if(!this.Data.favorites.filter(item=>item.ListingId == property.ListingId)[0]){
      this.Data.favorites.push(property);
      this.snackBar.open('The property "' + property.ListingId + '" has been added to favorites.', '×', {
